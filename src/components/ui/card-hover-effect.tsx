    ...
    <div className={cn('grid grid-cols-2  py-10  lg:grid-cols-3', className)}>
      {items.map(({ text, Icon }, idx) => (
        ...
        >
          ...
          <div className='relative z-20 w-full cursor-pointer overflow-hidden rounded-md bg-black p-4 ring-rose-500 transition-all duration-500 group-hover:ring-2'>
            <div className='relative z-50 space-y-5 py-10'>
              <Icon className='mx-auto h-8 w-8' />
              <p className='text-center text-2xl font-bold text-gray-300'>{text}</p>
            </div>
          </div>
        </div>
      ))}
